...
@mtechs.route('/edit_mtech/<application>', methods=['GET', 'POST'])
@login_required
def edit_mtech(application):
    if current_user.is_active == True:
        if current_user.is_manager==True or current_user.is_admin==True:
            ...
            def too(test):
                ...
                db_df=pd.read_sql('SELECT "'+var+'" FROM mtech WHERE application = "'+application+'"', conn)
                old_val=db_df[var][0]
